...
- [x] T12-fixes (from the three reviews; tick each as it lands)
...
| 25  | Double tap on "Terminé la vuelta"           | Accept · ignore repeats · ask to confirm                    | Ignore a repeat within 1.2 s and show "Vuelta N terminada · Deshacer" for 6 s                                                      | Protects against shaky double taps without adding a question                                                           |
| 26  | Feedback per stitch in rows mode            | Small line under the number · inside the tap area           | "12 puntos en esta vuelta" inside the tap area, with a short pulse                                                                 | The eyes are on the button while tapping                                                                               |
| 27  | "Quitar uno" at the start of a row          | Same label · explicit label                                 | "Volver a la vuelta N"                                                                                                             | Says exactly what will happen                                                                                          |
| 28  | Reaching a color/counter target             | Block like crochet · gentle notice                          | "¡Completado!" in green, counting continues                                                                                        | Embroiderers often need a few extra stitches                                                                           |
| 29  | Long forms                                  | Fixed button at the bottom · button after the questions     | After the questions (part forms only)                                                                                              | A fixed button hid the questions; people tapped it first                                                               |
| 30  | Where to rename/delete a project            | Small link in the header · big button                       | Big "Cambiar nombre o borrar proyecto" at the end of the parts screen                                                              | Findable, and out of the way of everyday counting                                                                      |
| 31  | Pause scope                                 | Whole app · per project                                     | Per project (Dexie v4)                                                                                                             | Pausing one labor must not lock the others                                                                             |
| 32  | Third tab without projects                  | Generic label · hidden                                      | Hidden until there is a project                                                                                                    | Avoids a wrong word ("Piezas") on the welcome screen                                                                   |
| 33  | Icons                                       | Keep · replace ambiguous ones                               | Folder (Proyectos), tally marks (Contar), hoop and needle (Bordado), sewing button (Otra cosa)                                     | The X and + shapes read as close/cancel/add                                                                            |
| 34  | Border contrast                             | Keep the soft design borders · ≥ 3:1                        | Borders of controls ≥ 3:1; disabled buttons dashed and muted                                                                       | Controls must be visible for older eyes                                                                                |
| 35  | App name (owner feedback)                   | Cuentapuntos · Punto a Punto                                | Cuentapuntos (replaces "Mis Labores")                                                                                              | Owner preferred these two; "Punto a Punto" clashes with the "Punto" technique label                                    |
| 36  | Database (owner feedback)                   | Keep old name and migrations · fresh schema                 | Fresh `cuentapuntos` database, single version, migrations removed                                                                  | Still in development; data loss is acceptable and the code is simpler (supersedes #12 and #20)                         |
